...
## 2026-09-16T15:35:34+08:00 · DOC-MIGRATE-stage-04 · 阶段记录迁移
...
- 历史任务 `AUT-0400`：原状态 `todo`，原阶段 `4`；原清单无附加记录。

- 历史任务 `AUT-0401`：原状态 `todo`，原阶段 `4`；原清单无附加记录。

- 历史任务 `AUT-0402`：原状态 `todo`，原阶段 `4`；原清单无附加记录。

- 历史任务 `AUT-0403`：原状态 `todo`，原阶段 `4`；原清单无附加记录。

- 历史任务 `AUT-0404`：原状态 `todo`，原阶段 `4`；原清单无附加记录。

- 历史决定：DEC-003；2026-08-30；使用区域式半开放地图；降低无缝大世界和加载复杂度；已由 DEC-010 和目标契约 v2 取代；Superseded

- 历史决定：DEC-010；2026-08-30；RC1 世界采用高密度手工核心地区、分块流送、确定性程序化外围和稀疏差量存档；同时保证手工密度、长期探索和大型世界可维护性；取代四区域上限；Stage 4 建立生成器、chunk 生命周期和性能证据；Accepted
