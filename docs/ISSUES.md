...
| ART3-003 | Medium | 3 | In Progress | 已有AG3/AG4真实RGBA技术候选，但仅下右静态；尚无通过四方向/配饰/可动接缝审查的正式母图 | AG4清洁链哈希、Alpha、四底和GPU静态检查通过；H0/M1且不批准PLAYER_MASTER；补面运动探针暴露ART3-006接缝 | 保留AG4原图和不批准状态，改用可控分层/网格方法完成结构与另外三个独立方向；复核真实尺度、四底及运动极值 | 有可编辑、来源清楚的多方向角色母图和已验证动作接缝，通过独立视觉审查后明确批准范围 |
| ART3-004 | Medium | 3 | In Progress | 官方Blender工具和小规模正交预渲染A/B已实际执行，但两路线绘本观感均未通过、正式路线未定 | 4.5.13 LTS实际运行；cutout/Blender固定相机与比例的21代表帧和Compatibility实景对照见Stage3 LOG与A/B REVIEW，双方存在High视觉缺陷 | 修正最小样件的结构/材质并量化一致性、动作成本和服装扩展，不扩建两条全量产线；保留失败截图 | ART3-100在相同场景尺寸下有过视觉门的样板、成本/一致性/扩展证据和书面路线决定 |
...
级别：Medium。Stage：3。状态：In Progress。

主屋、桥、水车和码头的现有材质/结构仍是原型样板；码头近景有较强木纹对比和首尾重复木结，绳索/几何柱体层次不足。2026-09-23桥/码头/水车共用新柔化木材shader且真实Compatibility近景已捕捉，桥/码头各一块固定木板灰度波动约18.33→13.82、18.42→13.76；仅说明局部对比降低，重复大木结和板/柱形体问题仍在。[制作与限制](design/SOFT_PAINTED_WOOD_PROBE.md)。独立审查只允许功能原型集成，不批准正式美术；继续在统一整景调整木纹密度、结构与左上光，通过三轮独立整体截图审查后关闭，不能以自动测试或单块统计替代。
...
### ENV3-003 — 四氛围仍只有静态视觉探针

级别：Medium。Stage：3。状态：In Progress。任务：ART3-102。

2026-09-23同一实际家园场景的固定相机四氛围GPU截图已取得；初次CanvasModulate方案漏过unshaded地面/水/房屋着色器，夜/晨地面取样亮度比1.0，保留失败图。现用世界层乘色与局部窗光修正，五个固定材质/角色样本夜/晨比约0.48；[设计审查边界](design/HOME_ATMOSPHERE_PROBE.md)详述。雨痕静止且无湿面/水体响应，黄昏木纹过暖，夜景灯具尚无批准设计，环境原型形体问题ENV3-001/002未关闭。下一步同镜头解决材质与结构、补雨/夜可读性和减少动态，经过三轮独立视觉审查及正式入口/分辨率证据后方可关闭；四张可区分截图不等于完整天气或四季系统。

### NPC3-001 — 首轮动物居民不是主NPC母图

级别：Medium。Stage：3。状态：In Progress。任务：ART3-101。

2026-09-23四张纯文字原创居民候选取得真RGBA与四底/144px首轮检查；[审查](../art/candidates/npc_river_residents_v001/REVIEW.md)暂列河狸A为改稿方向、乌龟D为备选。A仍偏直立写实和相机偏低，B偏动物肖像，C在游戏尺寸细腿/配件难读，D有隐藏足且低体态与主角接近。唯一A父图的A2第二轮改稿让笔触更宽、配件更清楚，但没有修到目标俯角/隐藏结构，保持未批准。A/A2已有同画布同相机144px真实Compatibility GPU静态对照，临时脚底pivot/接触影不能证明接地。仍无独立批准、四方向母图、编辑源或动态动画；不得把四物种当一只NPC的四方向。下一步换可控结构/转面方法，完成解剖/色板/道具和实景动态质量门后才可关闭；关系、日程与故事线另按后续Stage实现。

### UI3-004 — 正式9-slice面板与整套界面美术未完成

级别：Medium。Stage：3。状态：In Progress。任务：CAP-0520。

2026-09-26两张原创可编辑SVG面板候选已走精确源hash/512px真Alpha/Godot渲染与真实Compatibility暂停/设置界面检查；[审查](../art/candidates/ui_storybook_frame_v001/REVIEW.md)记录A的切片边距压缩内容及角饰过大，B减小角饰、56px纹理切片+24px内容边距后在中文1280×720与英文1280×800/150%可用。B仍偏平整矢量，未获得独立绘本美术批准，未进入`game/assets`或正式Theme；图标/字体、标题/交互提示、四分辨率和实体手柄/高对比矩阵未完成。只有通过完整设计/来源/布局/输入与两轮视觉门后才可关闭，不以PNG真Alpha或两张截图单独通过。
...
级别：High。Stage：1/2。状态：Closed。任务：ST1-R001。
